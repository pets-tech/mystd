...
class forward_list {
 ...
 private:
  ...
  class iterator_basic {
   ...
   public:
    ...
    reference operator*() const { return list_ptr->storage[current_index].data; }
 ...
 public:
  ...
  forward_list(forward_list&& other) noexcept
      : storage(std::move(other.storage)), head(other.head), free_head(other.free_head), size_(other.size_) {
  ...
  // element access
  void push_front(const value_type& data) { emplace_front(data); }
  void push_front(value_type&& data) { emplace_front(std::move(data)); }
