...
private:
    ...
    public:
        ...
        explicit iterator_basic(list_type* owner, node_type* ptr = nullptr)
        ...
        iterator_basic& operator--() {
            if (!ptr_) {
                if (!owner_ || !owner_->tail_)
                    throw std::out_of_range("Cannot decrement end() of empty list");
                ptr_ = owner_->tail_;
            } else {
                ptr_ = ptr_->prev;
            }
        ...
        iterator_basic operator++(int) { auto tmp = *this; ++(*this); return tmp; }
        iterator_basic operator--(int) { auto tmp = *this; --(*this); return tmp; }
